...
"""An event handler return value which can tigger an action or switch active handlers.

If a handler is returned then it will bevome the active handler for future events.
If an acction is returned it will be attempted and if it's valid then
...
class BaseEventHandler(tcod.event.EventDispatch[ActionOrHandler]):
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        ...
        if isinstance(state, BaseEventHandler):
            return state
        assert not issinstance(stat, Action), f"{self!r} can not handle actions."
        return self
...
CURSOR_Y_KEYS = {
    tcod.event.K_UP: -1,
    tcod.event.K_DOWN: 1,
    tcod.event.K_LEFT: 10,
    tcod.event.K_RIGHT: -10,
}
